...
<Submenu name="2">
            ...
            <Layout :style="{padding: '0 50px'}">
                <Breadcrumb :style="{margin: '16px 0'}">
                    ...
                    <BreadcrumbItem to="/agtl">讨论社区</BreadcrumbItem>
                    <BreadcrumbItem>{{indexxw.zhuti}}</BreadcrumbItem>
                    
                </Breadcrumb>
                <Content style="padding:50px 0; min-height:900px; background:#fff;width:1400px;margin-left: auto;margin-right: auto;">
                    <Row>
        <Col span="18" offset="3"> 
        <p style="font-size:26px;color:black"><Tag type="dot" :color="changeData()">{{indexxw.tab}}</Tag>&nbsp;{{indexxw.zhuti}}</p>


        
        <p style="float:left;padding:15px 0;font-size:14px;color:#999">{{indexxw.name}}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;发布自 {{indexxw.date}}</p>
        <p style="float:right;padding:8px;font-size:14px;">
            ...
            <Button @click="dingbbc(1)" :loading="loadingding" shape="circle" style="margin-right:5px;" icon="md-arrow-round-up">{{indexxw.dintie[0]}}</Button>
            <Button @click="dingbbc(0)" :loading="loadingding" shape="circle" style="margin-right:5px;" icon="md-arrow-round-down">{{indexxw.dintie[1]}}</Button>
            <Button @click="fxewm()" shape="circle" target="_blank" type="error" style="margin-right:5px;" >分享</Button>
            </ButtonGroup>
        </p>
        <Divider /><br/>
        <div class="vhtml" v-html="indexxw.data"></div></Col>
...
import QRCode  from "qrcodejs2"
 export default {
        ...
        created: function () {
           ...
           this.$http({
          method: "POST",
          url: 'https://ubfvxa4c.engine.lncld.net/1.1/functions/getbbcnr',
          json: true,
          headers: {
          ...
          that.$Loading.finish();
            ...
            dingbbc(sdsc){
                this.loadingding=true
                 ...
                 this.$http({
          method: "POST",
          url: 'https://ubfvxa4c.engine.lncld.net/1.1/functions/dcxsddd',
          json: true,
          headers: {
               "Content-Type": "application/json",
               "X-LC-Key": "vMPQ9lzIPvPHosJGxywekiOJ",
         ...
         if(sdsc){
             that.indexxw.dintie[0]+=1
         }else{
             that.indexxw.dintie[1]+=1
         }
          that.loadingding=false
     });
            },
            //  生成二维码
            qrcode () {
      ...
      this.modalxsd = true 
          ...
          headers: {
               ...
               'xiaoxi':this.$route.query.bbcdatec,
               'mishi':this.$mishi()
          }
     }, function (err, res, body) {
        ...
        if( jiegg.jieguo==0){
            ...
            ////////////
                ...
                var bodyxs={"urlpath":this.$route.query.bbcdatec,"name":this.formData.name,"email":this.formData.url,"data":this.formData.desc,"date":dategs}
                const that = this 
                this.$http({
          method: "POST",
